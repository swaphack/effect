...
namespace Assets.Editor.GameDesign.Terrains
{
    ...
    public class Terrain2MeshWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            terrainObj = Selection.objects[0] as GameObject;
            if (terrainObj == null)
            {
        ...
        void Export()
        {
            ...
            Vector2 uvScale = new Vector2(1f / (w - 1f), 1f / (h - 1f)) * vertexCountScale * (size.x / terrainData.terrainLayers[0].tileSize.x);     // [dev] 此处有问题，若每个图片大小不一，则出问题。日后改善
